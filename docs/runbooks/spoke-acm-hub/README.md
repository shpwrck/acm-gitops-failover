...
# hub-y standalone ACM hub leave-behind

## Operability

### State and access

Durable state lives on the `hub-y` OpenShift cluster (SNO, OCP 4.21.20,
api.hub-y.k8socp.com), reached via kubeconfig context `hub-y` in
`~/.kube/config` (cluster-admin `kube-admin` credentials embedded there; no
other secret store involved). The former hub is context `hub-x`
(api.hub-x.k8socp.com, its own MCH 2.17.0, now managing only `local-cluster`);
`spoke` exists but was powered off throughout.

- hub-y, namespace `open-cluster-management`: OperatorGroup
...
- hub-y was DETACHED from the old hub 2026-08-12 (clean detach: delete of
  `ManagedCluster hub-y` on `hub-x`, ~4.5 min cascade). Side effect confirmed
  live: the detach pruned all hub-delivered workloads on hub-y (namespace
...
- Console: no separate ACM route in 2.17 — OpenShift console
  (`console-openshift-console.apps.hub-y.k8socp.com`) → "All Clusters"
  perspective.
- Deliverable this state serves: the verified failover guide in this
  repo (`README.md` map, `docs/build.md`, `docs/exercises/`) and
  `research-notes.md` (doc citations).

### Template map

~/acm-failover-guide/manifests/10-acm-operator.yaml -> hub-y ns open-cluster-management + OperatorGroup + Subscription advanced-cluster-management (release-2.17)
~/acm-failover-guide/manifests/20-multiclusterhub.yaml -> hub-y MultiClusterHub multiclusterhub in ns open-cluster-management (availabilityConfig Basic)

### Re-run

Prerequisites: context `hub-y` reachable with cluster-admin; hub-y NOT a
managed cluster of any other hub (no foreign `klusterlet` CR — if attached,
first `oc --context <that-hub> delete managedcluster hub-y` and wait for the
`open-cluster-management-agent*` namespaces to vanish from hub-y).

```bash
oc --context hub-y apply -f ~/acm-failover-guide/manifests/10-acm-operator.yaml
# wait: CSV advanced-cluster-management.v2.17.0 = Succeeded (~3 min), then:
oc --context hub-y apply -f ~/acm-failover-guide/manifests/20-multiclusterhub.yaml
...
```bash
oc --context hub-y get mch -n open-cluster-management   # Running / 2.17.0 / "All hub components ready."
oc --context hub-y get managedclusters                  # local-cluster Joined=True Available=True
oc --context hub-y get klusterlet                       # exactly one (self-managed)
oc --context hub-y get pods -n open-cluster-management --no-headers | grep -v 'Running\|Completed'   # empty
oc --context hub-y adm top node                         # was 11% CPU / 42% mem with hub installed
```
...
hub-y into the old hub via its console (Import cluster) if the previous
...
- hub-y became its OWN ACM 2.17 hub (user decision) — step 1 of the
...
- `availabilityConfig: Basic` because hub-y is single-node (documented SNO
  setting; High doubles replicas for no benefit).
- Clean detach (delete ManagedCluster on hub) over hub-y-side manual
  klusterlet removal: hub-x returned after its restart, so the supported path
...
`oc --context hub-y apply -f <file>`, verify with "Verify and recover", and
record every verified step with real outputs in
the guide docs (`docs/build.md`, `docs/exercises/`) — the guide is the product; this runbook is
...
likely importing `spoke` into this new hub (hub-y) as the test subject. Commit
